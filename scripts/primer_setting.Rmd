...
```{r dataloading}
#加载数据并初步处理
setwd("C:/Users/Hongxi/Documents/R_project")
F_DNA <- paste(readLines("18S_rRNA_N1.txt"), collapse = "")
...
这里使用的限制条件有：  
  ...
  2. CG含量  
  3. 引物Tm:  
  引物的Tm值的计算方式为非常粗略的`Tm = 4(G+C)+2(A+T)`,后续优化可选择其他的公式，可能选用Merck提供的最邻近法公式：
    !["Tm最邻近法公式"](~/R_project/Tm_calculation.png){width=50%}   
  4. 引物自身3端配对  
  5. 引物与靶标的3端配对  
...
##计算CG含量(此处应该可以优化)
F_primer_all$GC <- unlist(lapply(as.list(F_primer_all$seq), GC_contain))
R_primer_all$GC <- unlist(lapply(as.list(R_primer_all$seq), GC_contain))

##根据GC阈值筛选
F_primer <- subset(F_primer_all, subset = GC >= GC_min & GC<=GC_max)
R_primer <- subset(R_primer_all, subset = GC >= GC_min & GC<=GC_max)
...
##计算Tm值：暂时使用Tm = 4(G+C)+2(A+T)计算（可优化)
F_primer$Tem <- 2*(F_primer$GC/100 + 1)*(F_primer$end - F_primer$start)

R_primer$Tem <- 2*(R_primer$GC/100 + 1)*(R_primer$end - R_primer$start)

##根据Tm值筛选

F_primer <- subset(F_primer, subset = Tem >= Tm_min & Tem <= Tm_max)
R_primer <- subset(R_primer, subset = Tem >= Tm_min & Tem <= Tm_max)
...
F_primer <- subset(F_primer,
                   ...
                   select = c("accession","seq","start","end","GC","Tem"))
...
F_primer <- subset(F_primer,
                   subset = tail_match == 1,
                   select = c("accession","seq","start","end","GC","Tem"))
colnames(F_primer) <- c("F_accession", "F_seq", "F_start", "F_end","F_GC","F_Tem")

R_primer <- subset(R_primer,
                   subset = tail_match == 1,
                   select = c("accession","seq","start","end","GC","Tem"))
colnames(R_primer) <- c("R_accession", "R_seq", "R_start", "R_end","R_GC","R_Tem")
...
get_all_primer_sets <- function(F_primer,R_primer,target_size,pcr_min,pcr_max,deltar_Tm,end_match, total_match){
  ...
  get_primer_sets <- function(f_primer_data,r_primer_data, target_size, pcr_min, pcr_max, deltar_Tm) {
      ...
      primer_sets$D_Tm <- primer_sets$R_Tem - primer_sets$F_Tem
      ...
      primer_sets <- subset(primer_sets, 
                            ...
                            & D_Tm <= deltar_Tm 
                            & D_Tm >= -deltar_Tm,
                              select = c("R_seq", "R_start", "R_end","R_GC","R_Tem", "F_seq", "F_start", "F_end","F_GC","F_Tem","pcr_length"))
...
```
以上表格随机显示了结果中的30对可用引物。  
  
## 部分结果的可视化
```{r painting, warning=FALSE}
library(ggplot2)
...
## 输出结果到结果目录
```{r output}
write.csv(primer_sets,file = "~/R_project/primer_sets.csv",row.names = F)
```